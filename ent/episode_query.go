...
func (eq *EpisodeQuery) QuerySeason() *SeasonQuery {
	query := (&SeasonClient{config: eq.config}).Query()
...
func (eq *EpisodeQuery) QueryCastMembers() *CastMemberQuery {
	query := (&CastMemberClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EpisodeQuery) Clone() *EpisodeQuery {
	...
	return &EpisodeQuery{
		...
		withSeason:      eq.withSeason.Clone(),
		withCastMembers: eq.withCastMembers.Clone(),
...
func (eq *EpisodeQuery) WithSeason(opts ...func(*SeasonQuery)) *EpisodeQuery {
	query := (&SeasonClient{config: eq.config}).Query()
...
func (eq *EpisodeQuery) WithCastMembers(opts ...func(*CastMemberQuery)) *EpisodeQuery {
	query := (&CastMemberClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withCastMembers = query
	return eq
}
...
func (eq *EpisodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Episode, error) {
	...
	if query := eq.withSeason; query != nil {
		if err := eq.loadSeason(ctx, query, nodes, nil,
			func(n *Episode, e *Season) { n.Edges.Season = e }); err != nil {
			return nil, err
		}
	}
	if query := eq.withCastMembers; query != nil {
		if err := eq.loadCastMembers(ctx, query, nodes,
			func(n *Episode) { n.Edges.CastMembers = []*CastMember{} },
			func(n *Episode, e *CastMember) { n.Edges.CastMembers = append(n.Edges.CastMembers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EpisodeQuery) loadSeason(ctx context.Context, query *SeasonQuery, nodes []*Episode, init func(*Episode), assign func(*Episode, *Season)) error {
...
func (eq *EpisodeQuery) loadCastMembers(ctx context.Context, query *CastMemberQuery, nodes []*Episode, init func(*Episode), assign func(*Episode, *CastMember)) error {
	...
	query.Where(predicate.CastMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(episode.CastMembersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.episode_cast_members
		if fk == nil {
			return fmt.Errorf(`foreign-key "episode_cast_members" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "episode_cast_members" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
